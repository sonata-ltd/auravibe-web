...
export const Spinner = (props: SpinnerProps) => {
	...
	const [enableSpin, setEnableSpin] = createSignal<boolean>(
		props.forceIndeterminate || true,
	);
	...
	createEffect(() => {
		const newSize =
			typeof props.size === "number" ? props.size : DEFAULT_SIZE;
		setSize(newSize);
	});
	...
	createEffect(() => {
		setMaxValue(
			props.maxValue !== undefined ? props.maxValue : DEFAULT_MAX_VALUE,
		);
	});
	...
	const isIndeterminate = () =>
		value() === undefined || props.forceIndeterminate;
	...
	const svgStyle = () => {
		...
		if (typeof props.style === "string") {
			...
			console.warn(
				"Spinner: String style prop is less flexible. Consider using CSSProperties object.",
			);
	...
	createEffect(() => {
		if (props.showValueText && isIndeterminate()) {
			console.warn(
				"Spinner: 'showValueText' is true, but 'value' is not provided. Text will not be displayed.",
			);
		}
	});
		...
		>
			<svg
				...
				classList={{
					[css.indeterminateAnimation]:
						isIndeterminate() && enableSpin(),
					[css.linearAnimation]:
						isIndeterminate() && props.linear && enableSpin(),
					[css.easeAnimation]:
						isIndeterminate() && !props.linear && enableSpin(),
			...
			>
				...
				<circle
					...
					stroke-dasharray={strokeDasharray().toString()} // Circumference for progress, CSS var for indeterminate
					stroke-dashoffset={strokeDashoffset()} // Calculated offset for progress
				/>
			</svg>

			<Show when={isCompleting() && !props.showValueText}>
				<div
